...
model_paths = {
    "hrnet-w32": "hrnetv2_w32_imagenet_pretrained.pth",
}
    ...
    def load_param(self, pretrained_path):
        if not Path(pretrained_path).exists():
            raise FileNotFoundError(
                f'HRNet-W32-C pretrained weights not found under "{pretrained_path}", please download it '
                f"first at https://github.com/HRNet/HRNet-Image-Classification or specify the correct "
                f"weights dir location with the cfg.model.backbone_pretrained_path config."
            )
